...
function loadVideos() {
  ...
  .then(data => {
    if (!data.error) {
      ...
      document.querySelector('.youtube-note').innerHTML = '<p class="youtube-noquote-note"><span style="color:darkred">Включена квотонепотребляемая версия</span> Youtube ICESHNILD, так как сегодня люди много сюда заходили за 1 день, и теперь не отображается статистика и сортировка видео.</p>'
